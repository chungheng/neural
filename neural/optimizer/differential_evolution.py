"""
GPU-Accelerated differential_evolution: The differential evolution
global optimization algorithm accelerated using CUDA.

Based on original script added by Andrew Nelson in 2014.
...
class OptimizationHistory:
    ...
    def __repr__(self):
        return f"Optimization History with {self.x.shape[0]} Steps"
...
class DifferentialEvolutionSolver(object):
    ...
    def __init__(
        ...
        if updating in ["immediate", "deferred"]:
            ...
            raise ValueError(
                f"differential_evolution: 'updating' keyword got value {updating}"
                " , needs to be 'immediate' or 'deferred."
            )

        # check for batched processing
        self.batched = batched
        if batched and workers != 1:
            warnings.warn(
                "differential_evolution: 'batched' and 'workers'"
                " keyword cannot both be True."
                " overridden workers to 1",
                UserWarning,
            )
            workers = 1

        if batched and updating == "immediate":
            warnings.warn(
                "differential_evolution: the 'batched' keyword has"
        ...
        self._desc = f"[DE {self._updating} {self.parameter_count}]"
        if verbose:
            self._pbar = tqdm(desc=self._desc, total=maxiter)
    ...
    def converged(self):
        ...
        return np.std(self.population_energies) <= self.atol + self.rtol * np.abs(
    ...
    def solve(self):
        ...
        history = OptimizationHistory(
            self.maxiter + 1, self.population.shape[0], self.population.shape[1]
        )
        ...
        if np.all(np.isinf(self.population_energies)):
            ...
            history.update(0, self.population, self.population_energies)
            if self.verbose:
                self._desc = (
        ...
        for nit in range(1, self.maxiter + 1):
            ...
            try:
                next(self)
                history.update(nit, self.population, self.population_energies)
    ...
    def _calculate_population_energies(self, population):
        ...
        if self.batched:
            calc_energies = self.func(parameters_pop[0:nfevs].T)
            if len(calc_energies) != nfevs:
                raise ValueError(
                    f"Batched processing returned cost of length {len(calc_energies)}"
                    f" , expect {nfevs}. Check if cost function handles batched"
                    " Parameters correctly, parameters are given as shape"
                    " (len(x), np.size(population, 0))"
                )
    ...
    def __next__(self):
        ...
        if self._updating == "immediate":
            ...
            loc = np.array(loc)
            # update solutions that lead to improvements
