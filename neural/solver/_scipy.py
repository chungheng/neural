...
class SciPySolver(BaseSolver):
    ...
    def set_initial_value(self, t0:float = 0, **initial_states):
        ...
        y0 = self._states_to_vec({
            var: np.repeat(val, self.model.num) if (np.isscalar(val) or val.size == 1) else val
            for var, val 
            in {
                **self.model.initial_states, 
                **initial_states
    ...
    def _vec_to_states(self, vec: np.ndarray, ref_state: dict = None) -> dict:
        return {
            ...
            for var, arr
            in zip(
                (ref_state.keys() if ref_state is not None else self.model.states.keys()),
    ...
    def get_wrapped_ode(self) -> None:
        def wrapped_ode(t, y, **input_args):
            self.model.states.update(
                self._vec_to_states(y, self.model.gstates)
            )
    ...
    def step(self, d_t: float, **input_args) -> None:
        ...
        self._t += d_t
        self.model.states.update(self._vec_to_states(
            self._solver.y if self._solver.t == self._t else self._dense_output(self._t), 
            self.model.gstates
        ))
