...
def rk_step(fun, states, gstates, d_t, A, B, C, K):
    ...
    states : ndarray, shape (num, n,)
        Current state.
    gstates : ndarray, shape (num, n,)
    ...
    K : ndarray, shape (num, n_stages + 1, n)
    ...
    """
    for _K, _y, _f in zip(K, y, f):
        _K[0] = _f
        for s, (a, c) in enumerate(zip(A[1:], C[1:]), start=1):
            dy = np.dot(_K[:s].T, a[:s]) * h
            _K[s] = fun(t + c * h, _y + dy)

        _y += h * np.dot(_K[:-1].T, B)
        _f[:] = fun(t + h, _y)
...
class RungeKutta(OdeSolver):
    ...
    def __init__(
        ...
        **extraneous
        ...
        self.f = self.fun(self.t, self.y)  # current gradient
