...
ARRAYS = {
    "numpy": {
        "scalar": np.float_(0.0),
        "0d": np.array(0.0),
        "1d": np.array([0.0]),
        "2d": np.array([[0.0]]),
    },
    "cupy": {
        "scalar": cupy.asarray(np.float_(0.0)) if cupy is not None else None,
        "0d": cupy.asarray(np.array(0.0)) if cupy is not None else None,
        "1d": cupy.asarray(np.array([0.0])) if cupy is not None else None,
        "2d": cupy.asarray(np.array([[0.0]])) if cupy is not None else None,
    },
    "pycuda.gpuarray": {
        "scalar": gpuarray.to_gpu(np.array(0.0)) if gpuarray is not None else None,
        "0d": gpuarray.to_gpu(np.array(0.0)) if gpuarray is not None else None,
        "1d": gpuarray.to_gpu(np.array([0.0])) if gpuarray is not None else None,
        "2d": gpuarray.to_gpu(np.array([[0.0]])) if gpuarray is not None else None,
    },
}
...
def test_get_array_module():
    for mod, arrays in ARRAYS.items():
        for key, arr in arrays.items():
            if arr is None:
                continue
            assert sys.modules[mod] == get_array_module(arr), f"{mod}, {key}"
        ...
        for key, arr in arrays.items():
            ...
            try:
                np.testing.assert_array_almost_equal(
                    cpu_arr, np.asarray(ARRAYS["numpy"][key])
                )
            except:
                print(cpu_arr)
